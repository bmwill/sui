...
//! # Liveness
//!
//! The collector holds a [`DbRef`](crate::DbRef) (weak handle), so
//! registering it does not keep the underlying database open. If
//! every strong [`Db`](crate::Db) handle has been dropped by the
//! time Prometheus scrapes, every gauge is reset to
//! [`METRICS_ERROR`](crate::RocksMetrics) (`-1`) so dashboards
//! see the database is gone rather than stale-looking values.
//!
//! # Sampling cadence
//!
    ...
    fn collect_after_db_drop_emits_metrics_error_sentinel() {
        ...
        for family in &families {
            for metric in family.get_metric() {
                ...
                for label in metric.get_label() {
                    if label.name() == "cf_name"
                        && cf_names.iter().any(|cf| *cf == label.value())
                    {
                        matched_cf = true;
                    }
                }
